...
func IsAuthorized(tokenStr string) (*AccessDetails, error) {
...
func CalcPages(docs, pgSize int64) (int64, error){

	if docs < 1 || pgSize  < 1 {
		return 0, fmt.Errorf("CalcPages:72 -- both Docs and PageSize must be > 0")
	}
	var pages int64
	x  :=  float64(docs)/float64(pgSize)
	//xt := math.Trunc(x)
	it := int64(x)
	//log.Debugf("CalcPages:78 -  Pages needed: %f,  config Page size: %f,  xt: %f, it: %d\n", x, float64(pgSize), xt, it)
	rndPages := math.Round(x)
	if x == float64(pgSize) {
		//log.Debug("Exact fit")
		pages = int64(rndPages)
	} else {
		//log.Debug("Need extra partial page")
		pages = it + 1
	 }
	 return pages, nil
